...
export default class DashboardComponent_wasla extends React.Component {
    ...
    componentWillMount() {
        ...
        fetch(url).then(function(response) {
            ...
            .then(function(data) {
                that.setState({ klant: data.klant });
            });
    }

    render() {

        const klantdashboard = Object.keys(this.state.klant.weight).map((content, idx) => {
            const items = this.state.klant.weight[content].map((item, i) => (
                <p key={i}>{item.text}</p>
            ))
